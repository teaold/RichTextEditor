...
@interface YSEditArticleVCtrl ()<UITextViewDelegate,UIWebViewDelegate,KWEditorBarDelegate,KWFontStyleBarDelegate,HXAlbumListViewControllerDelegate>
...
@implementation YSEditArticleVCtrl
...
- (UIWebView *)webView{
    if (!_webView) {
        ...
        htmlCont = [htmlCont stringByReplacingOccurrencesOfString:@"replacemessage" withString:self.contentStr];
        //加载页面
        [_webView loadHTMLString:htmlCont baseURL:baseURL];
        //设置编辑标题
//        [_webView setupTitle:self.titleString];
...
- (void)viewDidLoad {
    ...
    [self BarLeft:NO ItemStr:@"保存" Tcolor:COLOR_WORD_BLACK Tfont:16];
    
}
//保存

- (void)barRightAction:(UIButton *)btnRight {
    self.contentStr = [self.webView contentHtmlText];
    if (self.contentStr.length > 0) {
        WS(weakSelf)
        if (weakSelf.onChangeStateBlock) {
            weakSelf.onChangeStateBlock(self.contentStr);
        }
        [self.navigationController popViewControllerAnimated:YES];
    } else {
        [HUDView showErrorWithStatus:@"编辑内容不能为空!"];
    }
}
...
- (void)viewDidLayoutSubviews{
    ...
    self.webView.frame = CGRectMake(0, SCREEN_Y, self.view.frame.size.width, self.view.frame.size.height - SCREEN_Y - SCREEN_B_0);
}
...
- (void)albumListViewController:(HXAlbumListViewController *)albumListViewController didDoneAllList:(NSArray<HXPhotoModel *> *)allList photos:(NSArray<HXPhotoModel *> *)photoList videos:(NSArray<HXPhotoModel *> *)videoList original:(BOOL)original{
    ...
    if (photoList.count > 0) {
        for (HXPhotoModel *photoM in photoList) {
            //插入imagedata
            [self uploadImage:photoM.thumbPhoto];
            }
        }
}

- (void)uploadImage:(UIImage *)headImage {
    [HUDView show];
    NSMutableDictionary *dic = [[NSMutableDictionary alloc]init];
    [YSJsonDataModel requestSendContentImgDic:dic image:[Utils reSizeImageData:headImage maxImageSize:1.0] Complated:^(id datas, NSString *error) {
        [HUDView dismiss];
        if (error) {
            [HUDView showErrorWithStatus:error];
        } else {
            NSString *picUrl = datas;
#warning todo
            //[self.webView insertImage:picUrl];
        }
    }];
}
