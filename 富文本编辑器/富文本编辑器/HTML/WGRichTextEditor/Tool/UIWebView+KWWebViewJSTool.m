...
//    [self stringByEvaluatingJavaScriptFromString:@"RE.calculateEditorHeightWithCaretPosition();"];
    return [[self stringByEvaluatingJavaScriptFromString:@"RE.calculateEditorHeightWithCaretPosition();"] floatValue];
}
...
- (void)setupEditEnable:(BOOL)enable {
    //可编辑
    [self stringByEvaluatingJavaScriptFromString:enable ? @"RE.canFocus(true);" : @"RE.canFocus(false);"];
}
